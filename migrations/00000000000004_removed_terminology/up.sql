...
CREATE TABLE workspaces_renamed (
    ...
    state TEXT NOT NULL CHECK (
        state IN ('creating', 'ready', 'degraded', 'failed', 'removed')
    ),
...
CREATE TABLE repo_worktrees_renamed (
    ...
    state TEXT NOT NULL CHECK (
        state IN ('pending', 'attached', 'dirty', 'missing', 'diverged', 'failed', 'removed')
    ),
    last_head TEXT,
...
SELECT id, canonical_path, CASE state WHEN 'reclaimed' THEN 'removed' ELSE state END,
       created_at, updated_at, last_reconciled_at, management_mode, pool_id,
       last_released_at, reclaimed_at
FROM workspaces;

INSERT INTO repo_worktrees_renamed
SELECT id, workspace_id, origin_repository_id, worktree_path,
       CASE state WHEN 'reclaimed' THEN 'removed' ELSE state END,
